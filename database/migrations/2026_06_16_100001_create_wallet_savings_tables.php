...
return new class extends Migration
{
    ...
    {
        // Shared hosting MySQL (strict sql_mode) rejects NOT NULL timestamp
        // columns without an explicit default. Guard with hasTable because
        // earlier tables in this migration may already exist after a failed run.
        if (! Schema::hasTable('wallet_savings_settings')) {
        ...
        if (! Schema::hasTable('wallet_savings_goals')) {
        ...
        if (! Schema::hasTable('wallet_savings_locks')) {
            Schema::create('wallet_savings_locks', function (Blueprint $table) {
                ...
                $table->timestamp('matures_at')->useCurrent();
