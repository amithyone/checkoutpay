    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.account-numbers.store') }}" method="POST" id="account-number-form" onsubmit="return validateForm()">
            ...
            <div class="space-y-6">
                ...
                <div id="website-select" class="{{ (old('is_pool') || old('is_invoice_pool') || old('is_membership_pool') || old('is_tickets_pool')) ? 'hidden' : '' }}">
                    ...
                    <select name="business_website_id" id="business_website_id" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        ...
                        @foreach($websites as $website)
                            <option value="{{ $website->id }}" data-business-id="{{ $website->business_id }}" {{ old('business_website_id') == $website->id ? 'selected' : '' }}>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_external" id="is_external" value="1" {{ old('is_external') ? 'checked' : '' }} class="rounded border-gray-300 mr-2">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const businessSelect = document.getElementById('business_id');
        if (businessSelect) {
            businessSelect.addEventListener('change', filterWebsitesByBusiness);
        }
        const externalCheckbox = document.getElementById('is_external');
        if (externalCheckbox) {
        ...
        // If a bank is already selected (from old input), show it in search input
        if (bankCodeSelect && bankCodeSelect.value) {
    ...
    function selectBank(code, name) {
        ...
        // Trigger change event for account validation
        bankCodeSelect.dispatchEvent(new Event('change'));
        
        // If account number is already entered, trigger validation
        const accountNumberInput = document.getElementById('account_number');
    ...
    (function() {
        ...
        // Update bank name when bank is selected (for the hidden select)
        if (bankCodeSelect) {
        ...
        accountNumberInput.addEventListener('input', function() {
            ...
            // Validate if account number is exactly 10 digits (try with or without bank code)
            if (accountNumber.length === 10) {
                const bankCode = bankCodeSelect ? bankCodeSelect.value : '';
                
                // Ensure bank_name is set if bank code is selected
                if (bankCode && !bankNameInput.value) {
                    updateBankName();
                }
                ...
                // Debounce validation (API supports validation without bank code)
        ...
        function validateAccountNumber(accountNumber, bankCode) {
            ...
            .then(data => {
                if (data.success && data.valid) {
                    ...
                    if (data.bank_name && bankNameInput) {
                        ...
                        if (bankCodeSelect && data.bank_code) {
                            ...
                            for (let i = 0; i < options.length; i++) {
                                ...
                                if (optionBankName.toLowerCase() === data.bank_name.toLowerCase()) {
                                    bankCodeSelect.value = options[i].value;
                                    bankCodeSelect.dispatchEvent(new Event('change'));
                                    break;
                                }
                    ...
                    // Clear account name and bank name on validation failure
                    if (accountNameInput) {
                        accountNameInput.value = '';
                        accountNameInput.classList.add('bg-gray-50');
