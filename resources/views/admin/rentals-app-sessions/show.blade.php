...
@section('title', 'Rentals app session')
@section('page-title', 'Rentals app session detail')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.rentals-app-sessions.index') }}" class="text-sm text-gray-600 hover:text-gray-900">&larr; All sessions</a>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold text-gray-900">{{ $session->admin_name ?? $session->admin?->name ?? 'Admin' }}</h3>
                <p class="text-sm font-mono text-gray-600 mt-1">{{ $session->admin_email }}</p>
            </div>
            @if($session->isActive())
