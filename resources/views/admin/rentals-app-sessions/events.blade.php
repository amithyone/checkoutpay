...
@section('title', 'Rentals app session events')
@section('page-title', 'Rentals — App activity')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Login, logout, and idle expiry events across rentals admin app sessions.</p>
        </div>
        <a href="{{ route('admin.rentals-app-sessions.index') }}" class="text-sm text-primary hover:underline">View sessions &rarr;</a>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <form method="GET" action="{{ route('admin.rentals-app-sessions.events') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div class="sm:col-span-2">
                <label class="block text-xs font-medium text-gray-700 mb-1">Search</label>
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Email, IP, or summary…"
            ...
            <div class="flex items-end gap-2 lg:col-span-5">
                ...
                <a href="{{ route('admin.rentals-app-sessions.events') }}" class="text-sm text-gray-600 py-2">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($events as $event)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 font-mono text-xs">{{ $event->admin_email ?? '—' }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right">
                                @if($event->session)
                                    <a href="{{ route('admin.rentals-app-sessions.show', $event->session) }}" class="text-primary hover:underline text-xs">Session</a>
                                @endif
                            </td>
                        </tr>
                    @empty
