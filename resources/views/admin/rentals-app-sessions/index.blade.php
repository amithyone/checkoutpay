...
@section('title', 'Rentals app sessions')
@section('page-title', 'Rentals — App sessions')

@section('content')
<div class="space-y-6">
    @include('admin.rentals.partials.nav')

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h3 class="text-lg font-semibold text-gray-900">App sessions</h3>
            <p class="text-sm text-gray-600 mt-1">
                Rentals admin app sign-ins. Sessions are created on the server at login even when the app does not send a session id.
            </p>
        </div>
        <div class="flex gap-2 text-sm">
            ...
            <a href="{{ route('admin.rentals-app-sessions.events') }}" class="inline-flex items-center px-3 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <form method="GET" action="{{ route('admin.rentals-app-sessions.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div>
                ...
                <select name="login_method" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    <option value="">All</option>
                    <option value="password" @selected(request('login_method') === 'password')>Email &amp; password</option>
                </select>
            </div>
            <div class="flex items-end gap-2 lg:col-span-6">
                ...
                <a href="{{ route('admin.rentals-app-sessions.index') }}" class="text-sm text-gray-600 py-2">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($sessions as $s)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-900">{{ $s->admin_name ?? $s->admin?->name ?? '—' }}</div>
                                <div class="font-mono text-xs text-gray-500">{{ $s->admin_email }}</div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.rentals-app-sessions.show', $s) }}" class="text-primary hover:underline">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-gray-500">No rentals admin app sessions yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
