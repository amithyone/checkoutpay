...
@section('content')
<div class="p-6">
    <a href="{{ route('admin.rentals.index') }}" class="text-primary hover:underline mb-4 inline-block">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <p><strong>Name:</strong> <a href="{{ route('admin.businesses.show', $rental->business_id) }}" class="text-primary hover:underline">{{ $rental->business->name }}</a></p>
            <p><strong>Email:</strong> {{ $rental->business->email }}</p>
            @if($rental->business->phone)
                <p><strong>Phone:</strong> {{ $rental->business->phone }}</p>
            @endif
        </div>
        ...
        <div class="border-t pt-6">
            ...
            <form action="{{ route('admin.rentals.update-status', $rental) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="business_notes" rows="3" class="w-full border-gray-300 rounded-md">{{ $rental->business_notes }}</textarea>
                </div>
                <div class="flex gap-4">
                    <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary/90">
                        Update Status
                    </button>
                    <form action="{{ route('admin.rentals.destroy', $rental) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this rental?');">
                        @csrf
                        @method('DELETE')
