...
<div class="max-w-lg mx-auto bg-white rounded-lg border border-gray-200 p-6">
    ...
    @if($lenderCaps['max_amount'] < 1000)
        <div class="mb-4 p-3 bg-amber-50 border border-amber-200 text-amber-900 text-sm rounded-lg">
            Your effective maximum offer is below ₦1,000. Increase your balance or ask an administrator to adjust your lender limits.
        </div>
    @endif

    <form id="lendingOfferForm" method="POST" action="{{ route('business.lending-offers.store') }}" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700">Amount (₦)</label>
            <input type="number" name="amount" step="0.01" min="0.01" max="{{ number_format($lenderCaps['max_amount'], 2, '.', '') }}" value="{{ old('amount') }}" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
        ...
        <div>
            ...
            <input type="number" name="interest_rate_percent" step="0.01" min="0" max="{{ number_format($lenderCaps['max_interest'], 4, '.', '') }}" value="{{ old('interest_rate_percent') }}" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
        ...
        <div>
            ...
            <input type="number" name="term_days" min="1" max="{{ $lenderCaps['max_term'] }}" value="{{ old('term_days', max($lenderCaps['min_term'], min(30, $lenderCaps['max_term']))) }}" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
        ...
        <div>
            ...
            <select name="repayment_type" id="repayment_type" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" onchange="toggleFrequencyField()">
                <option value="lump" @selected(old('repayment_type', 'lump') === 'lump')>Lump sum at end of term — one full repayment on the last day</option>
                <option value="split" @selected(old('repayment_type', 'lump') === 'split')>Split — equal installments spread across the term (you choose how often)</option>
            </select>
            <p class="text-xs text-gray-500 mt-1">Example: a 30-day term + <strong>daily split</strong> = 30 equal slices (total repayment ÷ 30). <strong>Lump sum</strong> = a single payment on day 30.</p>
        </div>
        <div id="repayment_frequency_wrap" class="{{ old('repayment_type', 'lump') === 'split' ? '' : 'hidden' }}">
            ...
            <select name="repayment_frequency" id="repayment_frequency_select" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                <option value="daily" @selected(old('repayment_frequency') === 'daily')>Daily — one equal slice per day until the term ends</option>
                <option value="weekly" @selected(old('repayment_frequency', 'weekly') === 'weekly')>Weekly — about every 7 days</option>
                <option value="monthly" @selected(old('repayment_frequency') === 'monthly')>Monthly — every 30 days (same logic as collections)</option>
            </select>
        ...
        <button type="submit" id="lendingOfferSubmit" class="w-full py-2.5 bg-primary text-white rounded-lg text-sm font-medium disabled:opacity-60 disabled:cursor-not-allowed">Submit for approval</button>
    </form>
</div>
<script>
