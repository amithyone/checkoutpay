        ...
        <form action="{{ route('business.rentals.items.update', $item) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="brand" maxlength="120" placeholder="e.g. Sony, Canon" class="w-full border-gray-300 rounded-md" value="{{ old('brand', $item->brand) }}">
                </div>
            </div>
            ...
            <div class="border border-emerald-100 rounded-lg p-4 mb-4 bg-emerald-50/40">
                <div class="font-semibold text-emerald-900 mb-1">Discount</div>
                <label class="flex items-center gap-2 mb-3">
                    <input type="checkbox" name="discount_active" value="1" {{ old('discount_active', $item->discount_active) ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Discount active</span>
                </label>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium mb-1">Percent off (0–95)</label>
                        <input type="number" name="discount_percent" step="0.01" min="0" max="95" class="w-full border-gray-300 rounded-md" value="{{ old('discount_percent', $item->discount_percent) }}">
                    </div>
                    <div>
            ...
            <div class="border border-gray-200 rounded-lg p-4 mb-4">
                <div class="flex items-center justify-between">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="caution_fee_enabled" value="1" {{ old('caution_fee_enabled', $item->caution_fee_enabled) ? 'checked' : '' }} class="mr-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                    <div>
                        ...
                        <input type="number" name="caution_fee_percent" step="0.01" min="0" max="100" class="w-full border-gray-300 rounded-md" value="{{ old('caution_fee_percent', $item->caution_fee_percent) }}">
                    </div>
                </div>
            </div>
