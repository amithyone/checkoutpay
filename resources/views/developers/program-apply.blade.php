...
<div class="max-w-xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-midnight-deep mb-2">Apply to the program</h1>
        <p class="text-slate-600 text-sm sm:text-base mb-8">
            You must <strong class="text-midnight-deep">apply and be approved</strong> before any revenue share can accrue. After we review your application, we will follow up by email or WhatsApp. You will also be able to join our developer community on Slack or WhatsApp.
        </p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 sm:p-8">
            <form action="{{ route('developers.program.apply.store') }}" method="post" class="space-y-5">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">From your business dashboard. Leave blank only if you do not have an account yet—we will help you create one.</p>
                    @error('business_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <fieldset>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-surface-container-low has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="community" value="slack" class="text-primary focus:ring-primary" {{ old('community') === 'slack' ? 'checked' : '' }}>
                            <span class="text-sm text-midnight-deep"><i class="fab fa-slack text-purple-600 mr-1"></i> Slack</span>
                        </label>
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-surface-container-low has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="community" value="whatsapp" class="text-primary focus:ring-primary" {{ old('community') === 'whatsapp' ? 'checked' : '' }}>
                            <span class="text-sm text-midnight-deep"><i class="fab fa-whatsapp text-green-600 mr-1"></i> WhatsApp group</span>
                        </label>
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-surface-container-low has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="community" value="both" class="text-primary focus:ring-primary" {{ old('community') === 'both' ? 'checked' : '' }}>
                            <span class="text-sm text-midnight-deep">Both Slack and WhatsApp</span>
                        </label>
                    </div>
                    @error('community')
