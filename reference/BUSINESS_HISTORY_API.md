...
- `bank_transfer_out`, `vtu_*`, p2p, fees, savings locks, etc.

## Implementation

`ConsumerBusinessActivityService::paginate()` with `VIEW_ACCOUNT` filters wallet query to `business_rubies_in` only; still merges merchant payments and withdrawals.
